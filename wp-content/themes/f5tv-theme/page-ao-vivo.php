        ...
        <?php else: ?>
            <section class="grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_340px] gap-5">
                ...
                <aside class="rounded-3xl border border-zinc-900 bg-f5-blue-950 p-4 md:p-5">
                    ...
                    <div class="flex gap-2 mb-3"><input id="f5tv-channel-search" type="search" placeholder="Buscar canal" class="min-w-0 flex-1 rounded-lg border border-zinc-800 bg-black/20 px-3 py-2 text-xs text-white outline-none placeholder:text-zinc-600 focus:border-f5-red"><select id="f5tv-channel-filter" class="rounded-lg border border-zinc-800 bg-black/20 px-2 text-[10px] text-zinc-300 outline-none"><option value="all">Todos</option><?php $channel_categories = array_values(array_unique(array_map(static fn($item) => $item['category'], $channels))); foreach ($channel_categories as $category): ?><option value="<?php echo esc_attr(sanitize_title($category)); ?>"><?php echo esc_html($category); ?></option><?php endforeach; ?></select></div>
                    <div class="space-y-2" id="f5tv-channel-list">
                        <?php foreach ($channels as $index => $channel): ?><button type="button" class="f5tv-channel-btn w-full text-left p-3 rounded-xl border transition <?php echo $channel['id'] === $active_channel['id'] ? 'bg-f5-blue-900 border-f5-red/60' : 'bg-f5-blue-950 border-zinc-900 hover:border-zinc-700'; ?>" data-id="<?php echo esc_attr($channel['id']); ?>" data-stream="<?php echo esc_url($channel['stream']); ?>" data-name="<?php echo esc_attr($channel['name']); ?>" data-logo="<?php echo esc_attr($channel['logo']); ?>" data-category="<?php echo esc_attr($channel['category']); ?>" data-category-key="<?php echo esc_attr(sanitize_title($channel['category'])); ?>" data-live="<?php echo esc_attr($channel['live'] ? $channel['live']['title'] : ''); ?>" data-next="<?php echo esc_attr($channel['next'] ? $channel['next']['title'] . ' · ' . $channel['next']['start'] : ''); ?>" data-schedule="<?php echo esc_attr(wp_json_encode($channel['programs'])); ?>"><span class="flex items-center gap-3"><span class="w-9 h-9 rounded-lg bg-zinc-900 flex items-center justify-center text-[10px] font-black text-zinc-300"><?php echo esc_html($channel['logo']); ?></span><span class="min-w-0"><strong class="block text-xs text-zinc-200 truncate"><?php echo esc_html($channel['name']); ?></strong><small class="block text-[10px] text-zinc-600 font-mono mt-1"><?php echo esc_html($channel['live'] ? 'NO AR · ' . $channel['live']['title'] : ($channel['next'] ? 'PRÓXIMO · ' . $channel['next']['start'] : 'SEM GRADE')); ?></small></span><span class="ml-auto w-2 h-2 rounded-full shrink-0 <?php echo $channel['live'] ? 'bg-f5-red animate-pulse' : 'bg-zinc-700'; ?>"></span></span></button><?php endforeach; ?>
                    </div>
                    <p id="f5tv-channel-empty" class="hidden py-5 text-center text-xs text-zinc-600">Nenhum canal encontrado.</p>
                </aside>
            </section>
            ...
            <section class="mt-5 rounded-2xl border border-zinc-900 bg-f5-blue-950 p-5 md:p-6">
                <div class="flex flex-col md:flex-row md:items-end justify-between gap-3 mb-5"><div><span class="text-[10px] font-mono uppercase tracking-widest text-f5-red font-black">Guia ao vivo</span><h2 class="text-2xl font-black mt-1">O que está passando</h2></div><span class="text-[10px] font-mono uppercase text-zinc-600">Linha do tempo de hoje</span></div>
                <div class="space-y-3 overflow-x-auto pb-2">
                    <?php foreach ($channels as $channel): ?><div class="min-w-[680px] grid grid-cols-[150px_minmax(0,1fr)] gap-3 items-stretch"><div class="rounded-xl border border-zinc-900 bg-black/15 p-3"><strong class="block text-sm text-white truncate"><?php echo esc_html($channel['name']); ?></strong><span class="text-[10px] font-mono uppercase text-zinc-600"><?php echo esc_html($channel['category']); ?></span></div><div class="flex gap-2 min-h-[70px]"><?php if (!$channel['programs']): ?><div class="flex-1 rounded-xl border border-dashed border-zinc-800 p-3 text-xs text-zinc-600">Nenhuma programação cadastrada hoje.</div><?php else: foreach ($channel['programs'] as $program): $program_width = max(18, min(46, (($program['end_minutes'] - $program['start_minutes']) / 60) * 3)); ?><div class="rounded-xl border <?php echo $program['is_live'] ? 'border-f5-red/70 bg-f5-red/10' : 'border-zinc-900 bg-f5-blue-900/50'; ?> p-3" style="flex:0 0 <?php echo esc_attr($program_width); ?>%;"><div class="flex items-center justify-between gap-2"><span class="text-[10px] font-mono text-zinc-500"><?php echo esc_html($program['start'] . ' - ' . $program['end']); ?></span><?php if ($program['is_live']): ?><span class="text-[9px] font-mono font-black uppercase text-f5-red">No ar</span><?php endif; ?></div><strong class="block mt-2 truncate text-xs text-zinc-200"><?php echo esc_html($program['title']); ?></strong><span class="block mt-1 truncate text-[10px] text-zinc-600"><?php echo esc_html($program['host'] ?: 'Produção F5 TV'); ?></span></div><?php endforeach; endif; ?></div></div><?php endforeach; ?>
                </div>
            </section>
        <?php endif; ?>
    </div>
...
(function(){
    ...
    const channelButtons=[...document.querySelectorAll('.f5tv-channel-btn')];
    const search=document.getElementById('f5tv-channel-search');
    const filter=document.getElementById('f5tv-channel-filter');
    const empty=document.getElementById('f5tv-channel-empty');
    function filterChannels(){
        const query=(search?.value||'').toLowerCase().trim();
        const category=filter?.value||'all';
        let visible=0;
        channelButtons.forEach(item=>{const matchesText=(item.dataset.name||'').toLowerCase().includes(query)||(item.dataset.category||'').toLowerCase().includes(query);const matchesCategory=category==='all'||item.dataset.categoryKey===category;const show=matchesText&&matchesCategory;item.classList.toggle('hidden',!show);if(show)visible++;});
        empty?.classList.toggle('hidden',visible!==0);
    }
    search?.addEventListener('input',filterChannels); filter?.addEventListener('change',filterChannels);
